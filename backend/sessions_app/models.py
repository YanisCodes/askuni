...
class StudySession(models.Model):
    """A scheduled collaborative study session for a module chapter."""
    ...
    # Maps user_id → peer_id for WebRTC mesh networking while the session is live
    active_peer_ids = models.JSONField(default=dict, blank=True)
...
class ChatMessage(models.Model):
    """A chat message sent by a participant during a study session."""
...
class FocusScore(models.Model):
    """Focus tracking result for one participant in one study session."""

    session = models.ForeignKey(StudySession, on_delete=models.CASCADE, related_name='focus_scores')
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='focus_scores'
    )
